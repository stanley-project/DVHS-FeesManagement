/*
  # Update Schema for Villages and Bus Management

  1. New Tables
    - `villages`: Stores village information with distance from school
    - `bus_fee_structure`: Manages bus fees for each village
    - `student_academic_history`: Tracks student academic progression

  2. Changes to Existing Tables
    - Added village and bus service fields to Students table
    - Added category and type fields to Fee Types table
    - Added village-specific fields to Fee Structure table
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_villages_updated_at') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_student_academic_history_updated_at') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_bus_fee_structure_updated_at') THEN
