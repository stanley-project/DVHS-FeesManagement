/*
  # Add village and bus fee management

  1. New Tables
    - `villages`: Stores village information
      - `id` (uuid, primary key)
      - `name` (text, unique)
    ...
    - `bus_fee_structure`: Manages bus fees per village
      - `id` (uuid, primary key)
  ...
  2. Enums
    - `promotion_status`: Student promotion status options
    - `fee_category`: Types of fees (if not exists)

  3. Table Modifications
    - Add village and bus service fields to students table
    - Add category and monthly fee flags to fee_types table
    - Add village-specific fields to fee_structure table
