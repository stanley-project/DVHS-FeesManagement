/*
  ...
  1. Base Types and Functions
    - Creates update_updated_at_column() function
    - Creates all required ENUM types
    - Handles conditional type creation

  2. Core Tables
    - users table with phone validation and RLS
    - academic_years table with transition tracking
...
CREATE TABLE IF NOT EXISTS users (
  ...
  CONSTRAINT users_phone_number_check CHECK (phone_number ~ '^[0-9]{10}$'::text)
);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_academic_years_updated_at' AND tgrelid = 'academic_years'::regclass) THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_classes_updated_at' AND tgrelid = 'classes'::regclass) THEN
