/*
  ...
  1. Changes
     ...
     - Ensures one-time fees don't inflate the monthly fee amount
     - Adds more detailed debug information for troubleshooting

  2. Technical Details
     - Modifies the get_student_fee_status function to correctly calculate monthly school fees
     - Only includes recurring monthly fees in the monthly_school_fee value
     - Adds separate tracking for one-time fees vs recurring fees
*/
...
DECLARE
  ...
  v_recurring_monthly_fees NUMERIC := 0;
  v_debug_info JSONB;
...
BEGIN
  ...
  -- Get recurring monthly school fees - ONLY include fees marked as recurring monthly
  ...
  -- Calculate recurring monthly fees total
  v_recurring_monthly_fees := v_monthly_school_fee * v_months_passed_school;
  
  -- Calculate total school fees (recurring + one-time)
  v_total_school_fees := v_recurring_monthly_fees + v_one_time_school_fees;
  ...
  v_debug_info := jsonb_build_object(
    ...
    'recurring_monthly_fees', v_recurring_monthly_fees,
