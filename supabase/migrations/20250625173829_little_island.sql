-- First drop all functions that we're going to recreate
...
BEGIN
  ...
  student_data AS (
    SELECT 
      ...
      -- Calculate bus fees with monthly recurring
    ...
    GROUP BY s.id, s.class_id, s.has_school_bus, s.village_id
  ),
  class_summary AS (
    SELECT 
      cd.class_id,
...
BEGIN
  ...
  FOR student_record IN 
    SELECT s.id, s.student_name, s.admission_number, s.has_school_bus, s.village_id, s.registration_type
    FROM students s
...
-- Update fee_structure table to set is_recurring_monthly to true for all school fees
