/*
  ...
  1. Changes
     - Fixes the monthly school fee calculation to correctly handle fee types
     - Ensures only fees marked as recurring monthly are included in the monthly fee calculation
     - Adds detailed debugging information to help troubleshoot fee calculations
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'applicable_to_new_students_only', fs.applicable_to_new_students_only
  ))
  ...
  -- This is the key fix - we're ONLY including fees marked as is_recurring_monthly = true
  ...
  -- Calculate total school fees (monthly fee * months passed)
  -- This ensures we're only using the recurring monthly fees in the calculation
  ...
  v_debug_info := jsonb_build_object(
    ...
    'total_bus_fees_calculation', v_monthly_bus_fee || ' * ' || v_months_passed_bus || ' = ' || v_total_bus_fees,
