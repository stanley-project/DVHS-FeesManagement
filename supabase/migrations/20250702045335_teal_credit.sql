/*
  # Fix Payment Allocation with Equal Split Option

  This migration adds support for splitting payments equally between bus and school fees.
  
  1. Changes:
    - Drop and recreate the allocate_payment function with equal split option
    - Create a new RPC function for fee payment insertion with equal split parameter
    - Update the recalculate_all_payment_allocations function to handle equal splits
  
  2. Benefits:
    - Allows users to choose between priority-based allocation and equal split
    - Maintains backward compatibility with existing payments
    - Provides more flexibility in payment allocation
*/
...
-- Update the recalculate function to handle equal splits
