/*
  # Fix payment allocation with proportional split

  1. Changes
    - Add metadata column to fee_payments table
    - Replace allocate_payment function with improved version supporting proportional splits
    - Add RPC functions for payment processing with split options
    - Add function to recalculate payment allocations

  2. Features
    - Support for proportional payment allocation based on fee ratios
    - Support for equal split payment allocation
    - Support for standard allocation (bus fees first, then school fees)
    - Payment period tracking (current, advance, past)
*/
...
CREATE OR REPLACE FUNCTION calculate_months_between(start_date DATE, end_date DATE)
RETURNS INTEGER AS $$
DECLARE
  months INTEGER;
BEGIN
  months := (EXTRACT(YEAR FROM end_date) - EXTRACT(YEAR FROM start_date)) * 12 +
...
BEGIN
  ...
  ELSE
    -- Use CASE WHEN instead of ternary operator
    v_months_passed_bus := CASE WHEN v_student_record.has_school_bus THEN v_months_passed_school ELSE 0 END;
  END IF;
